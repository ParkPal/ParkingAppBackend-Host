...
def main():
	...
	this_lot.set_id(1)
#    this_lot.add_node(Node(1, "192.168.2.3"))
...
#    server_connection.init_conn()
	...
	while True:
		...
		mesh_connection = MeshConnection("192.168.2.1", 12345)
		mesh_connection.s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
		mesh_connection.init_conn()
...
#            server_connection.transmit_object(this_lot)
			...
			mesh_connection.s.shutdown(socket.SHUT_RDWR)
			mesh_connection.s.close()
#			break
		else:
